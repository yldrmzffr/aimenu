export const translations = {
  en: {
    title: "Smart Menu AI",
    description:
      "Discover our digital menu experience. Simply upload a photo of any menu and get instant, detailed information about every dish.",
    uploadTitle: "Upload Your Menu Photo",
    formats: "Accepted formats: JPG, JPEG, PNG, PDF",
    selectImage: "Choose Image",
    takePhoto: "Take Photo",
    maxSize: "Maximum size: 10MB",
    cancel: "Cancel",
    capture: "Capture",
    search: "Search dishes...",
    menuAssistant: "Smart Menu Guide",
    askAnythingAboutMenu:
      "Curious about a dish? Ask anything - ingredients, preparation methods, or dietary information.",
    askAboutMenu: "Ask About Menu",
    chatWelcomeMessage:
      "Welcome! I'm here to help you discover our delicious menu. What would you like to know?",
    chatErrorMessage: "An error occurred. Please try again.",
    menuItems: "Available Dishes",
    socketConnectionError: "Connection issue detected. Please try again.",
    uploadFailed: "Upload unsuccessful. Please try again.",
    tryAgainWithNewImage: "Try again with a new image",
    noMenuItems: "No menu items found.",
    menuItemsNotFound: "Menu or Menu Items not found.",
    backToHome: "Start Over",
    allItems: "All Items",
    generalInfo: "General Information",
    ingredients: "Ingredients",
    tellMeAbout: "Tell me about",
    preparation: "Preparation",
    howToPrepare: "How to prepare",
    nutrition: "Nutrition",
    nutritionInfo: "Nutrition information for",
    dietaryInfo: "Dietary Information",
    askAboutDish: "Ask About Dish",
    exploreMore: "Explore More",
    analyzingTitle: "Analyzing...",
    analyzing: "Analyzing... Please wait. This may take a few minutes.",
    analysisComplete: "Analysis complete!",
  },
  tr: {
    title: "Akıllı Menü",
    description:
      "Menümüzü dijital dünyada keşfedin. Menü fotoğrafını yükleyin, tüm yemekler hakkında anında detaylı bilgiye ulaşın.",
    uploadTitle: "Menü Fotoğrafını Yükle",
    formats: "Kabul edilen formatlar: JPG, JPEG, PNG, PDF",
    selectImage: "Fotoğraf Seç",
    takePhoto: "Fotoğraf Çek",
    maxSize: "Maksimum boyut: 10MB",
    cancel: "Vazgeç",
    capture: "Fotoğrafı Çek",
    search: "Yemeklerde ara...",
    menuAssistant: "Akıllı Menü Rehberi",
    askAnythingAboutMenu:
      "Yemekler hakkında aklınıza takılan her şeyi sorabilirsiniz - malzemeler, hazırlanış, kalori bilgisi ve daha fazlası.",
    askAboutMenu: "Menü Hakkında Sor",
    chatWelcomeMessage:
      "Hoş geldiniz! Lezzetli menümüzü keşfetmenize yardımcı olmak için buradayım. Ne öğrenmek istersiniz?",
    chatErrorMessage: "Bir hata oluştu. Lütfen tekrar deneyin.",
    menuItems: "Menüdeki Lezzetler",
    socketConnectionError: "Bağlantı sorunu oluştu. Lütfen tekrar deneyiniz.",
    uploadFailed: "Yükleme başarısız oldu. Lütfen tekrar deneyiniz.",
    tryAgainWithNewImage: "Yeni bir fotoğraf ile tekrar deneyin",
    noMenuItems: "Menüde yemek bulunamadı.",
    menuItemsNotFound: "Menü bulunamadı.",
    backToHome: "Yeniden Başla",
    allItems: "Tüm Ürünler",
    generalInfo: "Genel Bilgi",
    ingredients: "İçerik",
    tellMeAbout: "Bundan Bahset: ",
    preparation: "Hazırlanış",
    howToPrepare: "Nasıl hazırlanır",
    nutrition: "Besin Değeri",
    nutritionInfo: "Besin değerleri",
    dietaryInfo: "Diyet Bilgisi",
    askAboutDish: "Yemek Hakkında Sor",
    exploreMore: "Daha Fazla Keşfet",
    analyzingTitle: "Analiz Ediliyor...",
    analyzing:
      "Analiz ediliyor... Lütfen bekleyin. Bu birkaç dakika sürebilir.",
    analysisComplete: "Analiz tamamlandı!",
  },
  zh: {
    title: "智能菜单助手",
    description:
      "探索我们的数字化菜单体验。上传菜单照片，即刻获取每道菜品的详细信息。",
    uploadTitle: "上传菜单照片",
    formats: "支持格式：JPG、JPEG、PNG, PDF",
    selectImage: "选择图片",
    takePhoto: "拍摄照片",
    maxSize: "最大文件限制：10MB",
    cancel: "取消",
    capture: "拍摄",
    search: "搜索菜品...",
    menuAssistant: "智能菜单向导",
    askAnythingAboutMenu:
      "对菜品有任何疑问？可以询问原料、烹饪方法或营养信息等所有细节。",
    askAboutMenu: "询问菜单",
    chatWelcomeMessage: "欢迎！我将为您详细介绍我们的美味菜单。您想了解什么？",
    chatErrorMessage: "发生错误。请重试。",
    menuItems: "精选菜品",
    socketConnectionError: "连接出现问题，请重试。",
    uploadFailed: "上传失败，请重试。",
    tryAgainWithNewImage: "使用新图片重试",
    noMenuItems: "未找到菜品。",
    menuItemsNotFound: "未找到菜单或菜品。",
    backToHome: "返回首页",
    allItems: "所有菜品",
    generalInfo: "基本信息",
    ingredients: "原料",
    tellMeAbout: "介绍",
    preparation: "烹饪方法",
    howToPrepare: "如何准备",
    nutrition: "营养信息",
    nutritionInfo: "营养信息",
    dietaryInfo: "饮食信息",
    askAboutDish: "询问菜品",
    exploreMore: "查看更多",
    analyzingTitle: "分析中...",
    analyzing: "分析中... 请稍候。这可能需要几分钟。",
    analysisComplete: "分析完成！",
  },
  ar: {
    title: "القائمة الذكية",
    description:
      "اكتشف قائمتنا الرقمية. قم بتحميل صورة القائمة واحصل على معلومات فورية ومفصلة عن كل طبق.",
    uploadTitle: "تحميل صورة القائمة",
    formats: "الصيغ المدعومة: JPG، JPEG، PNG، PDF",
    selectImage: "اختيار صورة",
    takePhoto: "التقاط صورة",
    maxSize: "الحجم الأقصى: 10 ميغابايت",
    cancel: "إلغاء",
    capture: "التقاط",
    search: "البحث في الأطباق...",
    menuAssistant: "دليل القائمة الذكي",
    askAnythingAboutMenu:
      "هل لديك أي استفسار عن الأطباق؟ اسأل عن المكونات، طريقة التحضير، أو المعلومات الغذائية.",
    askAboutMenu: "اسأل عن القائمة",
    chatWelcomeMessage:
      "أهلاً بك! أنا هنا لمساعدتك في اكتشاف قائمتنا الشهية. ماذا تود أن تعرف؟",
    chatErrorMessage: "حدث خطأ. يرجى المحاولة مرة أخرى.",
    menuItems: "الأطباق المتوفرة",
    socketConnectionError: "حدثت مشكلة في الاتصال. يرجى المحاولة مرة أخرى.",
    uploadFailed: "فشل التحميل. يرجى المحاولة مرة أخرى.",
    tryAgainWithNewImage: "المحاولة مرة أخرى بصورة جديد ",
    noMenuItems: "لم يتم العثور على أطباق.",
    menuItemsNotFound: "لم يتم العثور على القائمة أو الأطباق.",
    backToHome: "العودة إلى الصفحة الرئيسية",
    allItems: "كل الأطباق",
    generalInfo: "معلومات عامة",
    ingredients: "المكونات",
    tellMeAbout: "أخبرني عن",
    preparation: "طريقة التحضير",
    howToPrepare: "كيفية التحضير",
    nutrition: "القيمة الغذائية",
    nutritionInfo: "معلومات غذائية لـ",
    dietaryInfo: "معلومات عن النظام الغذائي",
    askAboutDish: "اسأل عن الطبق",
    exploreMore: "استكشاف المزيد",
    analyzingTitle: "جاري التحليل...",
    analyzing: "جاري التحليل... يرجى الانتظار. قد يستغرق ذلك بضع دقائق.",
    analysisComplete: "اكتمل التحليل!",
  },
  ru: {
    title: "Умный Помощник Меню",
    description:
      "Откройте для себя цифровое меню. Загрузите фотографию меню и получите мгновенную подробную информацию о каждом блюде.",
    uploadTitle: "Загрузить фото меню",
    formats: "Поддерживаемые форматы: JPG, JPEG, PNG, PDF",
    selectImage: "Выбрать фото",
    takePhoto: "Сделать фото",
    maxSize: "Максимальный размер: 10МБ",
    cancel: "Отмена",
    capture: "Снять",
    search: "Поиск блюд...",
    menuAssistant: "Умный путеводитель по меню",
    askAnythingAboutMenu:
      "Интересует блюдо? Спрашивайте о чём угодно - ингредиенты, способ приготовления или информация о питании.",
    askAboutMenu: "Спросить о меню",
    chatWelcomeMessage:
      "Добро пожаловать! Я здесь, чтобы помочь вам открыть для себя наше вкусное меню. Что бы вы хотели узнать?",
    chatErrorMessage: "Произошла ошибка. Пожалуйста, попробуйте снова.",
    menuItems: "Доступные блюда",
    socketConnectionError:
      "Проблема с подключением. Пожалуйста, попробуйте снова.",
    uploadFailed: "Загрузка не удалась. Пожалуйста, попробуйте снова.",
    tryAgainWithNewImage: "Попробуйте снова с новым изображением",
    noMenuItems: "Блюда не найдены.",
    menuItemsNotFound: "Меню или блюда не найдены.",
    backToHome: "Начать заново",
    allItems: "Все блюда",
    generalInfo: "Общая информация",
    ingredients: "Ингредиенты",
    tellMeAbout: "Расскажите мне о",
    preparation: "Приготовление",
    howToPrepare: "Как приготовить",
    nutrition: "Питательная ценность",
    nutritionInfo: "Пищевая ценность для",
    dietaryInfo: "Информация о диете",
    askAboutDish: "Спросить о блюде",
    exploreMore: "Узнать больше",
    analyzingTitle: "Анализ...",
    analyzing:
      "Анализ... Пожалуйста, подождите. Это может занять несколько минут.",
    analysisComplete: "Анализ завершён!",
  },
  ja: {
    title: "スマートメニューAI",
    description:
      "デジタルメニュー体験をご堪能ください。メニューの写真をアップロードするだけで、各料理の詳細情報を即座に入手できます。",
    uploadTitle: "メニュー写真をアップロード",
    formats: "対応フォーマット：JPG、JPEG、PNG、PDF",
    selectImage: "画像を選択",
    takePhoto: "写真を撮影",
    maxSize: "最大サイズ：10MB",
    cancel: "キャンセル",
    capture: "撮影",
    search: "料理を検索...",
    menuAssistant: "スマートメニューガイド",
    askAnythingAboutMenu:
      "料理について気になることはありますか？材料、調理方法、栄養情報など、なんでもお尋ねください。",
    askAboutMenu: "メニューについて質問",
    chatWelcomeMessage:
      "ようこそ！美味しいメニューの紹介をお手伝いさせていただきます。何をお知りになりたいですか？",
    chatErrorMessage: "エラーが発生しました。もう一度お試しください。",
    menuItems: "提供中の料理",
    socketConnectionError: "接続に問題が発生しました。もう一度お試しください。",
    uploadFailed: "アップロードに失敗しました。もう一度お試しください。",
    tryAgainWithNewImage: "新しい画像で再試行",
    noMenuItems: "メニューアイテムが見つかりません。",
    menuItemsNotFound: "メニューまたはメニューアイテムが見つかりません。",
    backToHome: "最初からやり直す",
    allItems: "すべての料理",
    generalInfo: "基本情報",
    ingredients: "材料",
    tellMeAbout: "について教えて",
    preparation: "調理方法",
    howToPrepare: "調理手順",
    nutrition: "栄養",
    nutritionInfo: "の栄養情報",
    dietaryInfo: "食事制限情報",
    askAboutDish: "料理について質問",
    exploreMore: "もっと詳しく",
    analyzingTitle: "分析中...",
    analyzing: "分析中... お待ちください。数分かかる場合があります。",
    analysisComplete: "分析完了！",
  },
};
